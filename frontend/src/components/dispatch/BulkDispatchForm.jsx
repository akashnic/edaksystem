...
export function BulkDispatchForm({ selectedReceives, onSubmit, isLoading }) {
  ...
  const handleConfirm = () => {
    ...
    onSubmit({
      receives: selectedReceives.map(r => r.l_id),
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="bg-blue-50 p-4 rounded-md border border-blue-100 shadow-sm">
          <h3 className="text-sm font-semibold text-blue-900 mb-2">
            {t('SELECTED')} ({selectedReceives.length})
          </h3>
          <ul className="text-sm text-blue-800 space-y-1 max-h-32 overflow-y-auto list-disc pl-4">
            {selectedReceives.map(r => (
              <li key={r.l_id}>
                <strong>{r.letter_number}</strong>: {r.subject || 'No Subject'}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <form id="bulk-dispatch-form" onSubmit={handleSubmit} className="space-y-4">
      ...
      <ConfirmDialog
        ...
        message={t('CONFIRM_BULK_DISPATCH_MSG')}
